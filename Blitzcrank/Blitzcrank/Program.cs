...
namespace Blitzcrank
{
    class Program
    {
        ...
        private static void castQ(Obj_AI_Hero target)
        {
            ...
            else if (Config.Item("Hit").GetValue<Slider>().Value == 3 && target.Path.Count() < 2 && (Math.Abs((ObjectManager.Player.Distance(waypoints.Last<Vector2>().To3D()) - ObjectManager.Player.Distance(target.Position))) > 300 || target.Path.Count() == 0))
                Q.CastIfHitchanceEquals(target, HitChance.High, true);
